 ...
 *
 * @help
 * Note: Some modifcations to this plugin were made for Okitsune.
 * They're at the bottom of the file, below "▼ End of Plugin".
 * ===========================================================================
 *                              Translation Engine
 *                                 Version 1.04
 *                                  SumRndmDde
...
Game_System.prototype.initialize = function() {
	...
	this.applyLanguageFont();     // NEW – sets the correct font on boot
};
...
// -------------------------------------------------------------------------
// (New) Apply the correct default font whenever language changes
// -------------------------------------------------------------------------
Game_System.prototype.applyLanguageFont = function() {
  ...
  if (font) {
    this.setMessageFontName(font);           // YEP’s built-in setter :contentReference[oaicite:0]{index=0}
  } else {
...
Object.defineProperty(ConfigManager, 'language', {
	...
	set: function(value) {
		...
		$.translateWindowTitle();
		if ($gameSystem) $gameSystem.applyLanguageFont();   // NEW
	},
	configurable: true
});
...
Window_Options.prototype.languageStatusText = function(value) {
	return value === 0 ? '日本語' : $.languages[value - 1];
};
...
/* (New) --- Translation Engine × YEP Core/Message : language-aware global font --- */
(function ($) {
