 ...
 * @type string[]
 * @default []
 * @desc List entries like 'pictures_A1' or 'tilesets_Outside_C'. Keep '!'/'$' if present (e.g. 'characters_!$Gate0'). No extension.
 */
...
// pre-parsed outfit id/label lists for UI and checks
const OUTFIT_IDS  = {};   // chIdx -> number[]
const OUTFIT_META = {};   // chIdx -> [{id,label}]
...
CHANNELS.forEach(ch => {
  ...
  // Covered Images: normalize folder to lowercase; keep basename (with !/$ etc.) as-is
  const covered = JSON.parse(ch.coveredImages || '[]')
    .map(s => (typeof s === 'string' ? s : String(s)))
    .map(s => s.replace(/^\s+|\s+$/g, ''))   // hard trim
    .filter(Boolean)
    .map(s => {
      const i = s.indexOf('_');
      if (i > 0) {
      ...
      return s; // legacy (no folder) kept if your stable version expects it
    });
  ...
  JSON.parse(ch.outfits || '[]')
    ...
    .forEach(o => {
      ...
      if (id && pre) {
        ...
        if (!PREFIX_MAP[id]) PREFIX_MAP[id] = pre; // optional cross-channel alias
  ...
  // NEW: build fast lists for UI and lock checks (avoids repeated JSON.parse later)
  const ids  = [];
  const meta = [];
  JSON.parse(ch.outfits || '[]')
    .map(s => JSON.parse(s))
    .forEach(o => {
      const id  = Number(o.id || 0);
      const lbl = (o.prefix || `ID ${id}`);
      if (id) { ids.push(id); meta.push({ id, label: lbl }); }
    });
...
Game_System.prototype.refreshOutfitPrefixes = function(){
  ...
  this._outfitEligible = new Set();   // NEW: union of covered basenames for active channels

  const arr = $gameVariables.value(VAR_ID);
  if (!Array.isArray(arr)) return;

  // Walk from end to start (your original logic for priority)
  for (let i = arr.length - 1; i >= 0; i--) {
    const id = Number(arr[i] || 0);
    if (!id) continue;

    // Prefix list (unchanged behavior)
    const pre = PREFIX_MAP[`${i}:${id}`] || PREFIX_MAP[id];
    if (pre) {
      const tag = pre + '_';
      if (!this._outfitPrefixes.includes(tag)) this._outfitPrefixes.push(tag);
    }
...
// cache for existence checks, and project root path (MV/NW.js)
const FILE_EXISTS_CACHE = new Map();
const ROOT_DIR = (path && typeof process !== 'undefined' && process && process.mainModule)
  ? path.dirname(process.mainModule.filename)
  : '';

// Build safe variants for folder_basename keys (handles URL-encoding edge cases)
function _of_keyVariants(subfolder, fname) {
  ...
  // In practice these two cover the edge cases we care about:
  return [key, alt1];
}
...
ImageManager.loadBitmap = function (folder, filename, hue, smooth) {
  ...
  //  - filename: base name WITHOUT extension (MV passes that), and we KEEP leading !/$ if present.
  // Normalize the subfolder to lowercase; keep basename exactly as MV passes it
  // faster subfolder extraction + normalization
  ...
  // --- decision cache lookup (instant hit for repeats while outfits unchanged)
  ...
  // Build foldered key variants (handles URL-encoding like % → %25, ( → %28, ) → %29)
  const candidates = _of_keyVariants(subfolder, fname);

  let covered = false;
  if (eligible && eligible.size) {
    // Require explicit foldered key; ignore all unfoldered entries.
    covered = candidates.some(k => eligible.has(k));   // reuse computed variants
  }

  if (!covered) {
    DECISION_CACHE.set(decisionKey, null);  // remember: no override here
    return _loadBmp.call(this, folder, filename, hue, smooth);
  }

  // Covered → try active prefixes in priority order (unchanged behavior)
  const build = (pre, f) => {
    const m = /^([!$]{1,2})(.*)$/.exec(f);
...
ImageManager._okFileExists = function(folder, name){
  ...
  if (FILE_EXISTS_CACHE.has(key)) return FILE_EXISTS_CACHE.get(key);
...
Window_OutfitGrid.prototype.makeItemList=function(){
  ...
  list.forEach(o => this._data.push({ id:o.id, label:o.label, channel:this._ch }));
};
