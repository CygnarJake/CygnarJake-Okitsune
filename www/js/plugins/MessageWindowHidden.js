 ...
 * @type boolean
 *
 * @param OkitsuneUnhide
 * @desc If true, cursor keys + Escape can UNHIDE the window.
 * @type boolean
 * @default false
 *
...
(function() {
	...
	Window_Message.prototype.isTriggeredHidden = function() {
		...
		/* --- OkitsuneUnhide ------------------------------- */
		if (this.isHidden() && param.OkitsuneUnhide &&
			['up','down','left','right','escape'].some(function(sym){
				return Input.isTriggered(sym);
			})) {
			return true;
		}
		/* --- End OkitsuneUnhide --------------------------- */
