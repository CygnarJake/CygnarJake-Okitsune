...
(function() {
	...
	Scene_Boot.prototype.start = function() {
		// If the game restarted due to renderer change, skip the splash screen
		if (localStorage.getItem("Okitsune_Restart") === "true") {
			_Scene_Boot_start.call(this);
			// Now that it's been checked, remove it so it doesn't persist
			localStorage.removeItem("Okitsune_Restart");
			return;
		}

		if ((Liquidize.MadeWithMV.ShowMV || Liquidize.MadeWithMV.ShowCustom) && 
	...
	// Add a language check table at the top (add near your parameter reads)
	Liquidize.MadeWithMV.SplashLangs = {
		"日本語":    false,
		"English":  false,
		"Español":  false,
		"Português":false,
		"한국어":    true,   // Only Korean is true!
		"简体中文":  false
	};
	...
	Scene_Splash.prototype.createSplashes = function() {
		...
		} else if (ConfigManager.language) {
			lang = ConfigManager.language;
		}

		// Helper to create a splash sprite with lang check
		function createSplashSprite(base, lang) {
			var sprite = new Sprite();
			// Only check for a language-specific image if enabled
			if (lang && Liquidize.MadeWithMV.SplashLangs[lang]) {
				sprite.bitmap = ImageManager.loadSystem(base + "_" + lang);
			} else {
				sprite.bitmap = ImageManager.loadSystem(base);
			}
			return sprite;
		}

		if (Liquidize.MadeWithMV.ShowMV) {
			var baseName = Liquidize.MadeWithMV.MVImage;
			var sprite = createSplashSprite(baseName, lang);
