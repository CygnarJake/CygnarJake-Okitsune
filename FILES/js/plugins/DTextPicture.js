...
(() => {
    const _proc = Sprite_Picture.prototype._processCharacter;
    if (!_proc)
        return;
    Sprite_Picture.prototype._processCharacter = function (ts, bm) {
        if (ts.text[ts.index] === '\x1b') {
            const i = ts.index;
            const code = this.hiddenWindow.obtainEscapeCode(ts);
            if (code === 'C') {
                const s = this.hiddenWindow.obtainEscapeParamString(ts);
                const n = Number(s);
                bm.textColor = isNaN(n) ? s : this.hiddenWindow.textColor(n);
                return;
            }
            ts.index = i;
        }
        _proc.call(this, ts, bm);
    };
})();
