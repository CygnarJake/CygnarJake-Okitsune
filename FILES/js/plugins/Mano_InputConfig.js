...
class BasicSymbol extends I_SymbolDefine{
    ...
    isMandatory(){
        return !["escape", "pagedown"].includes(this._symbol);	//return true; 
    }
    helpText(){
...
function createBasicSymbols(){
    ...
    return [ok,cancel,shift,menu,pagedown,esacape];
}
...
class BasicSymbolList{
    ...
    constructor(ok,cancel,shift,pagedown,menu,shift_){
 ...
 * @param {String} nine 
 */
function createGamepadLayout(symbol,name,button0,button1,button2,button3,L1,L2,R1,R2,eight,nine){
    ...
    const buttons=[
        ...
        new GamepadButtonObj(8,eight),
        new GamepadButtonObj(9,nine),
        new GamepadButtonObj(10,"LS"),
        new GamepadButtonObj(11,"RS")
    ];
...
class DeviceXXX{
    ...
    * @param {Key_Layout} jis 
     * @param {Key_Layout} us 
     */
    setKeyLayout(jis,us,latam,pt){
        this._keyLayoutJIS=jis;
        this._keyLayoutUS =us;
		this._keyLayoutLATAM =latam;
		this._keyLayoutPT =pt;
    }
    onBoot(){
...
const setting = (function(){
    ...
    const nintendo=createGamepadLayout("N","任天堂","Y","B","A","X","L","ZL","R","ZR","-","+");
    const playstation=createGamepadLayout("P","プレイステーション","×","○","□","△","L1","L2","R1","R2","クリエイト","オプション");
    const xbox =createGamepadLayout("X","エックスボックス","A","B","X","Y","LB","LT","RB","RT","ビュー","メニュー");
    const numberGamepad =createButtonNumberLayout("Number","入力割り当て（上級）");

    const gamepadLayoutSelector = new LayoutSelecter([xbox,nintendo,playstation,numberGamepad]);
...
class Window_GamepadConfig_V8 extends Window_Selectable_InputConfigVer{
	...
	drawItem(index){
		...
		if(!item){return}
		let left = item.leftText();                  // <== make "left" a let
		const right = item.rigthText();
		const SpecialSelection = ["設定を保存","初期設定に戻す","ボタン表記変更","やめる"].includes(left);

		// === LANGUAGE-BASED ADJUSTMENT FOR "メニュー" ===
		if(left === "メニュー") {
			// Get current language (default to Japanese if undefined)
			const lang = (typeof ConfigManager.getLanguage === "function")
				? ConfigManager.getLanguage()
				: (ConfigManager.language || "日本語");

			// Map of what "メニュー" should display as per language
			const menuLabels = {
				"日本語": "メニュー",
				"English": "メニュー",
				"Español": "メニュー",
			...
			// Set left to mapped label, or default to original if not listed
			left = menuLabels[lang] !== undefined ? menuLabels[lang] : left;
		}

		const rect = this.itemRectWithPadding(index);
		this.changePaintOpacity(item.xxOpacity());
		const rightWidth = (!SpecialSelection) ? rect.width *0.7 : 0;
		this.drawText(left,rect.x,rect.y, rect.width - rightWidth);

		const colonW =this.textWidth(":");
		const rightX =rect.x + rect.width -rightWidth;

		if(!SpecialSelection){
			this.drawText(":",rightX,rect.y,rightWidth);

			if(!!right){
				this.drawText(right,rightX+colonW,rect.y,rightWidth);
			}
		}
	}
...
const KEYS ={
    ...
    ENTER_US:new Key_Big("Enter",13,3,1,true),
	ENTER_LATAM: new Key_Big('Enter',13,2,2,true),
    NULL:new Key_Null(),
    ...
    DELETE:keyinfo('Del',46),
    HOME:keyinfo('Home',36),
    END:keyinfo('End',35),
    PAGEUP:new Key_Locked('PgUp',33),
	...
	EQUAL_US:keyinfo('=',187),
    
    SQUARE_BRACKETS_OPEN :keyinfo('[',219),
    SQUARE_BRACKETS_CLOSE :keyinfo(']',221),
	
	//New Language Keys
	APOSTROFE:keyinfo("'",219),
	INTERROGACIÓN:keyinfo("¿",221),
	ACENTO:keyinfo("´",186),
	MÁS:keyinfo("+",187),
	Ñ:keyinfo("Ñ",192),
	LLAVER:keyinfo("{",222),
...
const KEY_LAYOUT_LATAM=(function(){ 
    const list =[
        KEYS.ESC,
        KEYS._1 ,
        KEYS._2 ,
    ...
    const layout= new Key_Layout("LATAM",list);
    layout.setEnterKey(KEYS.ENTER_LATAM);
    return Object.freeze( layout);
})();
const KEY_LAYOUT_PT=(function(){ 
    const list =[
        KEYS.ESC,
        KEYS._1 ,
        KEYS._2 ,
    ...
    const layout= new Key_Layout("PT",list);
    layout.setEnterKey(KEYS.ENTER_LATAM);
    return Object.freeze( layout);
})();
...
class Window_KeyConfig_MA extends Window_InputConfigBase {
	...
	  if (layoutText === "JIS") {
	    this._layout = KEY_LAYOUT_JIS;
	  } else if (layoutText === "LATAM") {
	    this._layout = KEY_LAYOUT_LATAM;
	  } else if (layoutText === "PT") {
	    this._layout = KEY_LAYOUT_PT;
	  } else { this._layout = KEY_LAYOUT_US; }
	}
    inputDevice(){
        return this._layout;
    }
	...
	  const current = this.getKeyLayout();
	  if (current === "JIS") {
	    this.setKeyLayout("US");
	  } else if (current === "US") {
	    this.setKeyLayout("LATAM");
	  } else if (current === "LATAM") {
	    this.setKeyLayout("PT");
	  } else {
