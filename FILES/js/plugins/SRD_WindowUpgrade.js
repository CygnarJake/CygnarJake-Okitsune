...
SRD.PluginCommands['setchoicewindowdata'] = function(args) {
	if (typeof $gameSystem.wu_info === "undefined") $gameSystem.wu_info = {};	//OKITSUNE: sometimes got a crash and had to add this. Not sure at the moment if I'll have to add it to other window commands here.
...
SRD.PluginCommands['createchoicewindow'] = function(args) {
	...
	for(let i = 1; i < args.length; i++) {
		argString += args[i]; // + ' ';		OKITSUNE: the extra space was causing an issue where the last entry had an incorrect space appended to the end.
	}
...
Window_ChoiceBase.prototype.setupWidth = function(width) {
	...
	this.width = 896; //this.windowWidth(); OKITSUNE 896 is temporary fix for choice window width. Current logic here doesn't seem to work, window width only affected by size of first entry, which isn't desired.
	this.refresh();
	this.select(0);
};
