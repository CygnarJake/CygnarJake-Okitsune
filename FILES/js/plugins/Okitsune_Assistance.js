...
// ** DTextPicture
//=============================================================================
 // Text color RGB/hex support for \C[...]
(function () {
    const _processCharacter = Sprite_Picture.prototype._processCharacter;
    if (typeof _processCharacter !== "function")
        return;
    Sprite_Picture.prototype._processCharacter = function (ts, bitmap) {
        if (ts.text[ts.index] === "\x1b") {
            const startIndex = ts.index;
            const code = this.hiddenWindow.obtainEscapeCode(ts);
            if (code === "C") {
                const param = this.hiddenWindow.obtainEscapeParamString(ts);
                const numeric = Number(param);
                bitmap.textColor = isNaN(numeric) ? param : this.hiddenWindow.textColor(numeric);
                return;
            }
            ts.index = startIndex;
        }
        _processCharacter.call(this, ts, bitmap);
    };
})();
//___________________________
// * Commented out a portion of the "Game_Screen.prototype.setDTextPicture" function in the main plugin file.
...
// SaveInfo – Actor State Display
(function () {
    Window_SaveInfo.prototype.drawState = function (pos, dx, dy, dw) {
        const actorId = this._saveContents.party._actors[pos - 1];
        const member = this._saveContents.actors._data[actorId];
        if (!member)
            return;
        ...
        const sorted = states.map(id => $dataStates[id]).filter(Boolean).sort((a, b) => a.priority - b.priority);
        const name = sorted[0] ? sorted[0].name : "-";
