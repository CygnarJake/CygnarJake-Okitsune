...
(function(){
  ...
  // Keep a reference to the original trigger checker
  var _okitsu_base_isTriggeredHidden = Window_Message.prototype.isTriggeredHidden;

  // Keys that should UNHIDE when the message window is currently hidden
  var _okitsu_unhideKeys = ['up','down','left','right','escape'];

  Window_Message.prototype.isTriggeredHidden = function(){
    // 1) Let the original logic run first (respects disable flags, triggerSwitch,
    //    right-click/shift/etc.). If it already decided, keep that decision.
    var baseResult = _okitsu_base_isTriggeredHidden.apply(this, arguments);
    if (baseResult) return true;

    // 2) Additional rule (always-on for your game):
    //    If the window is currently hidden and the plugin isn't disabled,
    //    let cursor keys or Escape UNHIDE it.
    if (this.isHidden() && !this.disableWindowHidden()) {
      for (var i = 0; i < _okitsu_unhideKeys.length; i++) {
        if (Input.isTriggered(_okitsu_unhideKeys[i])) return true;
      }
    }
