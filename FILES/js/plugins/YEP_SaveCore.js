...
Window_SaveInfo.prototype.updateIndex = function() {
  ...
  this._waitTime = 3;	// OKITSUNE: File preview load time in frames. Was 30, lowered it.
  this.contents.clear();
};
...
Window_SaveInfo.prototype.drawData = function(data, dx, dy, dw) {
  ...
  } else if (data.match(/STATE[ ]?(\d+)/i)) {	//OKITSUNE addition
    this.drawState(parseInt(RegExp.$1), dx, dy, dw);
  }
};

/* OKITSUNE: Old version of function commented out
...
Scene_File.prototype.onActionLoad = function() {
  // OKITSUNE: Only show confirmation window if in-game (not from title)
...
Scene_File.prototype.performActionSave = function() {
	// OKITSUNE: Set variable 94, know which file the player saved to
...
(function() {
  ...
  // Refresh: applies cropped background color and image per slot.
