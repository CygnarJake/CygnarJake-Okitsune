 ...
 * @param language
 * @text language
 * @desc Enter the language for this section (String).
 * @type string
 * @default 日本語
 */
...
(function () {
    ...
    function pickStructItem(parameter) {
        ...
        return {
			...
			language: pickStringValueFromParameter(parameter, 'language', '日本語'),	//OKITSUNE addition
		};
    }
    ...
    class Sprite_StaffRoll extends Sprite {
        ...
        createSection(section) {
			...
			// OKITSUNE: Expanded logic here to handle languages
            const currentLang = (typeof ConfigManager.getLanguage === "function") ? ConfigManager.getLanguage() : (ConfigManager.language || '日本語');
            section.items.filter(item => {
                if (!item.language)
                    return true;
                return item.language.split(',').map(x => x.trim()).includes(currentLang);
            }).forEach((item, j) => {
                if (section.title || j > 0)
                    this._totalHeight += Torigoya.EasyStaffRoll.parameter.designItemMargin;
        ...
        textFontFace() {
            ...
            let lang = (typeof ConfigManager.getLanguage === "function") ? ConfigManager.getLanguage() : (ConfigManager.language || '日本語');
            return STAFFROLL_FONT_TABLE[lang] || (typeof $gameSystem.mainFontFace === "function" ? $gameSystem.mainFontFace() : 'GameFont, sans-serif');
        }
