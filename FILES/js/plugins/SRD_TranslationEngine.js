...
/* OKITSUNE: Had crash issues, commented this out and made new version below.
...
$.ImageManager_loadBitmap = ImageManager.loadBitmap;
ImageManager.loadBitmap = function (folder, filename, hue, smooth) {
    if (filename && /_\[Original\]$/.test(filename) && !ConfigManager.isDefaultLanguage()) {
        const fs = require('fs');
        const path = require('path');
        const lang = ConfigManager.getLanguage();
        const translated = filename.slice(0, -11) + '_[' + lang + ']';
        const fullPath = path.join(process.mainModule.filename, '..', folder, translated + '.png');
        if (fs.existsSync(fullPath)) {
            return $.ImageManager_loadBitmap.call(this, folder, translated, hue, smooth);
        }
    }
    return $.ImageManager_loadBitmap.call(this, folder, filename, hue, smooth);
};
