 ...
 *
 * @param Language
 * @desc The language name – must match a name from the “Languages” array.
 * @default Spanish
 *
 * @param Font
 * @desc The font family (or comma-separated font list) you want to use when
 *       that language is selected.  Example: SimHei, Heiti TC, sans-serif
 * @default GameFont
 */
...
// OKITSUNE: renamed this to TranslationManager_1 from TranslationManager
// Honestly, I don't remember why I did, but I'm afraid of breaking it now.
function TranslationManager_1() {
	throw new Error('This is a static class');
}
...
// -------------------------------------------------------------------------
// OKITSUNE: language-to-font table
// -------------------------------------------------------------------------
$.langFonts = {};
try {
  var fontPairs = JSON.parse($.params['Language Font Pairs'] || '[]');
  fontPairs.forEach(function(obj) {
    var pair = JSON.parse(obj);          // each entry is a JSON string
    $.langFonts[pair.Language] = pair.Font;
  });
} catch (e) {
  console.warn('SRD_TranslationEngine:  Language Font Pairs could not be parsed.');
}
// -------------------------------------------------------------------------
...
Game_System.prototype.initialize = function() {
	...
	this.applyLanguageFont(); //OKITSUNE addition
};
...
// -------------------------------------------------------------------------
// OKITSUNE: Apply the correct default font whenever language changes
// -------------------------------------------------------------------------
Game_System.prototype.applyLanguageFont = function() {
  if (!$gameSystem || !$gameSystem.setMessageFontName) return;
  var lang = ConfigManager.getLanguage();
  var font = SRD.TranslationEngine.langFonts[lang];
  if (font) {
    this.setMessageFontName(font);
  } else {
    // fall back on YEP_MessageCore’s original init routine
    if (this.initMessageFontSettings) this.initMessageFontSettings();
  }
};
...
Object.defineProperty(ConfigManager, 'language', {
	...
	set: function(value) {
		...
		$.translateWindowTitle();
		if ($gameSystem) $gameSystem.applyLanguageFont();   //OKITSUNE addition
	},
	configurable: true
});
...
Window_Options.prototype.languageStatusText = function(value) {
	return value === 0 ? '日本語' : $.languages[value - 1];	//OKITSUNE: changed to 日本語 from English
};
...
Window_Command.prototype.addCommand = function(name, symbol, enabled, ext) {
	...
	var lang = ConfigManager.getLanguage();
	// console.log("::: " , lang);	OKITSUNE: Commented this out
...
})(SRD.TranslationEngine, TranslationBuilder, TranslationManager_1);
...
/* --- Translation Engine × YEP Core/Message : language-aware global font --- */
(function ($) {
  ...
  Game_System.prototype.applyLanguageFont = function () {
      ...
      var lang = ConfigManager.getLanguage();          // "English", "Español", …
      var font = $.langFonts[lang]                     // from Language Font Pairs
              || Yanfly.Param.MSGFontName              // YEP fallback
              || Yanfly.Param.DefaultFont              // Core fallback
              || 'GameFont';
      ...
      // Already-created bitmaps keep their own copy of fontFace.
      if (Bitmap) Bitmap.prototype.fontFace = font;
  };

  // --------------------------------------------------------------
  // Run once on boot
  // --------------------------------------------------------------
  const _alias_init = Game_System.prototype.initialize;
  Game_System.prototype.initialize = function () {
      _alias_init.apply(this, arguments);
...
(function() {
  ...
  // Patch the currencyUnit property
  Object.defineProperty(TextManager, "currencyUnit", {
    configurable: true,
    get: function() {
      var lang = (typeof ConfigManager.getLanguage === "function")
        ? ConfigManager.getLanguage()
        : (ConfigManager.language || "日本語");
      // If language not mapped, use Japanese by default:
      return currencyUnits[lang] || currencyUnits["日本語"];
    }
  });
})();

/* Make sure the language font is re-applied after loading a save */
(function() {
  const _extract = DataManager.extractSaveContents;
  DataManager.extractSaveContents = function(contents) {
      _extract.call(this, contents);          // restore all objects

      // apply the font that matches ConfigManager.language right now
      if ($gameSystem && $gameSystem.applyLanguageFont) {
          $gameSystem.applyLanguageFont();
      }
  };
...
(function() {
  ...
  ConfigManager.applyData = function(config) {
    ...
    // Only run on first boot / if language not set
    // (Default Language in SRD_TranslationEngine is already handled in original logic)
    if (
      (!config || config.language === undefined) &&
      typeof steamworks !== "undefined" &&
      typeof steamworks.getCurrentGameLanguage === "function"
    ) {
      var steamLang = steamworks.getCurrentGameLanguage();
      ...
      var langName = map[steamLang];

      // Only set if mapped and it's in the plugin's languages
		if (langName) {
		  if (
			SRD.TranslationEngine.languages &&
			SRD.TranslationEngine.languages.indexOf(langName) >= 0
		  ) {
			this.language = SRD.TranslationEngine.languages.indexOf(langName) + 1;
		  } else if (langName === SRD.TranslationEngine.sourceName) {
			this.language = 0;
		  }
		}
...
/* -------------------------------------------------------------------------
 * Memoization for $.translate(text)
 * Avoids re-running regex translations for repeated text.
 * ------------------------------------------------------------------------- */
(function(){
  // Guard if engine not present
  ...
  function getLanguageKey(){
    try {
      if (TE) {
        if (typeof TE.getLanguage === 'function') return String(TE.getLanguage());
        if (typeof TE.language     !== 'undefined') return String(TE.language);
        if (typeof TE._language    !== 'undefined') return String(TE._language);
      }
      if (typeof ConfigManager !== 'undefined') {
        if (typeof ConfigManager.language !== 'undefined') return String(ConfigManager.language);
        if (typeof ConfigManager.locale   !== 'undefined')  return String(ConfigManager.locale);
      }
    } catch(e){ /* ignore */ }
    return 'default';
  }
  ...
  window.__SRD_TE_TranslationCache = {
    ...
    max: function(n){ if (n > 0) MAX_CACHE = n|0; }
  };
})();
