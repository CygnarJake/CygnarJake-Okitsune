...
/*:
 * @plugindesc ※This is a modified version by CygnarJake for Nine-Tailed Okitsune Tale
...
// --- Allow tagged events with Through=ON to pass dontPassRegion ---
(function() {
  // Read the same parameter TMMoveEx uses
  var parameters = PluginManager.parameters('TMMoveEx');
  var dontPassRegionId = +(parameters['dontPassRegionId'] || 252);

  // Keep original behavior for everything else
  var _Game_Event_isMapPassable_Fix = Game_Event.prototype.isMapPassable;

  Game_Event.prototype.isMapPassable = function(x, y, d) {
    // If this event has the tag <Through_dontPassFIX> and Through is ON,
    // allow stepping into tiles marked with dontPassRegionId.
    if (this.isThrough && this.isThrough() &&
        this.loadTagParam && this.loadTagParam('Through_dontPassFIX')) {
      var x2 = $gameMap.roundXWithDirection(x, d);
      var y2 = $gameMap.roundYWithDirection(y, d);
      if ($gameMap.regionId(x2, y2) === dontPassRegionId) {
        return true;
      }
    }
    // Otherwise, use original logic.
    return _Game_Event_isMapPassable_Fix.call(this, x, y, d);
  };
})();
