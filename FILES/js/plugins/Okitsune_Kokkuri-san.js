 ...
 * -------------------------
 * Notes:
 * - Matching is exact (no whitespace trimming besides leading/trailing).
 * - Scripts run with access to MV globals, e.g. $gameVariables, $gameSwitches, etc.
 * - If a Script throws, it will be logged to the console.
 *
 * @param MaxLength
 * @type number
 * @min 1
 ...
 * @param Text
 * @type string
 * @desc The exact text that, when entered, triggers the Script.
 * @default
 *
 * @param Script
 * @type note
 * @desc JavaScript code to run when the Text matches on 「はい」. (No surrounding < > needed.)
 * @default ""
 */
...
(function () {
    ...
    // Column definitions (top-to-bottom text per column)
    const COLS = [
        "わ を ん", // 1) w, n column
        "らりるれろ", // 2) r column
        "や ゆ よ", // 3) y column
    ...
    function buildSymbolObjects() {
        ...
        // Top row (10 columns): [ _ , いいえ, _ x6 , はい, _ ]
        list.push(spacer()); // 0
        list.push(cell(TILE_NO)); // 1
        list.push(spacer()); // 2
        list.push(spacer()); // 3
        list.push(spacer()); // 4
        list.push(spacer()); // 5
        ...
        for (let row = 0; row < maxH; row++) {
            for (let c = 0; c < cols.length; c++) {
                const ch = cols[c][row];
                list.push(ch && ch !== " " ? cell(ch) : spacer()); // ← spaces are blanks
            }
        }
    ...
    Window_KokkuriBoard.prototype.drawItem = function (index) {
        ...
        const txt = s ? s.text : "";
        this.resetTextColor();
        this.changeTextColor(KOKKURI_TEXT_COLOR); // unified board color
    ...
    /*
    Window_KokkuriBoard.prototype.processOk = function() {
    if (this.isCurrentItemEnabled()) {
    this.updateInputData();
    // no deactivate; no auto OK sound — your handler plays feedback
    this.callOkHandler();
    } else {
