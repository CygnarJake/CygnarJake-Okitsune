...
(function () {
  ...
  const alias_origin_Window_Base_drawTextEx = Window_Base.prototype.drawTextEx;
  Window_Base.prototype.drawTextEx = function (text, x, y) {
    // OKITSUNE: Added below if conditional.
	if (Imported["SumRndmDde Translation Engine"]) {
        text = SRD.translate ? SRD.translate(text) : SRD.TranslationEngine.translate(text);
    }
	// End of addition.
    if (text) {
      this.resetFontSettings();
...
/* ====================================================================== */
/* Choice-window support for RS_MessageAlign                           */
/* ====================================================================== */
(function () {
  'use strict';

  // Keep the original in case another plug-in references it.
  const _RSMA_drawItem = Window_ChoiceList.prototype.drawItem;

  /**
   * Overwrite drawItem so each choice line can be left / center / right
   * according to \TA[n]  or  <CENTER> tags.
   */
  Window_ChoiceList.prototype.drawItem = function (index) {

    /* --------------------------------------------------------------
     * 1. Raw string exactly as typed in “Show Choices”.
     * ------------------------------------------------------------ */
    const raw   = $gameMessage.choices()[index] || '';
    let   align = 'left';                       // default

    /* --------------------------------------------------------------
     * 2. Detect & strip any alignment tag *before* escape-expansion.
     *    Handles  \TA[0|1|2]   and   <LEFT>/<CENTER>/<RIGHT>.
     * ------------------------------------------------------------ */
    const trimmed = raw
      .replace(/^[ \t]*(?:\\|¥)TA\[(\d)\]/i, (_, n) => {
      ...
      .replace(/^[ \t]*<(LEFT|CENTER|RIGHT)>/i, (_, dir) => {
        align = dir.toLowerCase() === 'center'
               ? 'center' : (dir.toLowerCase() === 'right' ? 'right' : 'left');
        return '';
      });

    /* --------------------------------------------------------------
     * 3. Expand *all* escape codes (icons, variables, colours, etc.)
     *    using RS/YEP-patched convertEscapeCharacters.
     * ------------------------------------------------------------ */
    const text = this.convertEscapeCharacters(trimmed);

    /* --------------------------------------------------------------
     * 4. Measure pixel width AFTER all codes are resolved.
     *    YEP_MessageCore already defines textWidthEx.
     * ------------------------------------------------------------ */
    const tw = Window_Base.prototype.textWidthEx.call(this, text);

    /* --------------------------------------------------------------
     * 5. Compute starting X based on alignment.
     * ------------------------------------------------------------ */
    ...
    /* --------------------------------------------------------------
     * 6. Draw the final processed text.
     * ------------------------------------------------------------ */
    this.resetTextColor();
  ...
  if (!_RSMA_drawItem)
    console.warn('RS_MessageAlign add-on: could not find original drawItem');
})();