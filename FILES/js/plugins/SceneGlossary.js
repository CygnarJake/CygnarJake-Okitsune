 ...
 * Add a screen that allows you to view the terms that appear in the game.
...
(function() {
	...
	/* OKITSUNE: Commented out this function, replaced with new version below.
	...
	*/
	...
	Game_Party.prototype.isSameGlossaryType = function(item) {
		var type = this.getSelectedGlossaryType();
		var meta = getMetaValues(item, ['種別', 'Type']);
		if (!meta) return type <= 1;
		// Split values by comma, then check each
		var types = meta.split(',').map(function(v) { return Number(v.trim()); });
...
/* ──────────────────────────────────────────────────────────────────────────
 * SceneGlossary - language-specific description tags
 *   • <SG説明:...> / <SGDescription:...>          ← source-language fallback
...
(function() {
  ...
  Window_Glossary.prototype.getDescription = function(pageIdx) {
      ...
      // 2) try language-specific meta tags first
      const desc = this.getMetaContents(
          [`説明_${lang}`, `Description_${lang}`],
          pageIdx
      );
      if (desc) return this._postProcessDescription(desc);

      // 3) otherwise fall back to the original behaviour
      return _origGetDesc.call(this, pageIdx);
  };

  // helper – run the same replacements the original method applies
  Window_Glossary.prototype._postProcessDescription =
    Window_Glossary.prototype._postProcessDescription ||
    function(text) {        // copy the processing block from original
        const data = this._itemData;
        text = text.replace(/\x1bEFFECT_TYPE\[(\d+)]/gi, () =>
                 Window_Glossary.effectCodeDesc[data.effects[+RegExp.$1].code]);
        ...
        text = text.replace(/\x1bCOMMON\[(\d+)]/gi, () => {
                 ...
                 const c = this.getCommonDescription(); this._itemData = data; return c;});
        if (this._enemy) text = this.convertEnemyData(text);
        return text;
    };
