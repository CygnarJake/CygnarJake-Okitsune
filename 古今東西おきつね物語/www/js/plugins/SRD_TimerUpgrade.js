...
Sprite_Timer.prototype.createBackground = function() {
	// OKITSUNE: Call these in scripts to change BG color.
	const b1 = SRD.TimerUpgrade.bg1 || 'rgba(0,0,0,0)';
	const b2 = SRD.TimerUpgrade.bg2 || 'rgba(0,0,0,1)';
	...
	if(_.back) {
		if(_.horz) {
			this.background.bitmap.gradientFillRect(0, 0, width, height, b1, b2, false);
			this.background.scale.x = _.horz;
		} else {
			this.background.bitmap = new Bitmap(width * 2, height);
			this.background.bitmap.gradientFillRect(0, 0, width, height, b1, b2, false);
			this.background.bitmap.gradientFillRect(width, 0, width, height, b2, b1, false);
...
Sprite_Timer.prototype.createLabel = function() {
	...
	this.label.bitmap.fontFace = Okitsune.TU_Param(_.font) || _.font;	//OKISTUNE
	this.label.bitmap.fontSize = _.labelSize;
	this.label.bitmap.fontItalic = _.italic;
	this.label.bitmap.textColor = Okitsune.TU_Param(_.color) || _.color;	//OKISTUNE
...
Sprite_Timer.prototype.createBitmap = function() {
	...
	bitmap.fontFace = Okitsune.TU_Param(_.font) || _.font;	//OKISTUNE
	...
	this.numBitmap.fontFace = Okitsune.TU_Param(_.font) || _.font;	//OKISTUNE
	this.numBitmap.fontSize = fontSize;
	this.numBitmap.fontItalic = _.italic;
	this.numBitmap.textColor = Okitsune.TU_Param(_.color) || _.color;	//OKISTUNE
...
Sprite_Timer.prototype.timerText = function() {
	// OKITSUNE: Formatting change, and set display to " ∞ " when % Infinity results in NaN form Haruchii's Kyo Omikuji.
	let result = Okitsune.TU_Param(_.format) || _.format;
	const frames = $gameTimer.getFrames();
	const seconds = Math.floor(frames / 60);
	const minutes = Math.floor(frames / (60 * 60));
	const isFiniteFrames = Number.isFinite(frames);
	const frm = isFiniteFrames ? frames % 60 : " ∞ ";
	const sec = isFiniteFrames ? seconds % 60 : " ∞ ";
	const min = isFiniteFrames ? minutes % 60 : " ∞ ";
	const hur = isFiniteFrames ? Math.floor(minutes / 60) % 60 : " ∞ ";
