...
(() => {
    ...
    Game_System.prototype.applyLanguageFont = function () {
        const lang = ConfigManager.getLanguage();
        let font = 'GameFont';
        const langFonts = SRD.TranslationEngine && SRD.TranslationEngine.langFonts;
        const yanflyParam = Yanfly && Yanfly.Param;
        if (langFonts && langFonts[lang]) {
            font = langFonts[lang];
        } else if (yanflyParam && yanflyParam.MSGFontName) {
            font = yanflyParam.MSGFontName;
        } else if (yanflyParam && yanflyParam.DefaultFont) {
            font = yanflyParam.DefaultFont;
        }
        this.setMessageFontName(font);
        if (yanflyParam)
            yanflyParam.DefaultFont = font;
    ...
    DataManager.extractSaveContents = function (contents) {
        ...
        if ($gameSystem && $gameSystem.applyLanguageFont) $gameSystem.applyLanguageFont();
    };
    ...
    Object.defineProperty(ConfigManager, 'language', {
        ...
        set(value) {
            ...
            if ($gameSystem && $gameSystem.applyLanguageFont) $gameSystem.applyLanguageFont();
            if (Okitsune && Okitsune.OutfitSystem && Okitsune.OutfitSystem.invalidateImageCache) Okitsune.OutfitSystem.invalidateImageCache();
            if (Okitsune && Okitsune.Dictionary && Okitsune.Dictionary.invalidateCache) Okitsune.Dictionary.invalidateCache();
        }
    });
    ...
    ConfigManager.applyData = function (config) {
        ...
        if (config && config.language !== undefined) return;
...
(function () {
    Window_SaveInfo.prototype.drawState = function (pos, dx, dy, dw) {
        ...
        const state = states.map(id => $dataStates[id]).find(s => s && s.iconIndex === 33);
...
(function () {
    ...
    Window_MessageBacklog.prototype.fullDeactivate = function () {
        ...
        if (this._okitsuneOpenedDuringPause && $gameMessage) {
			...
			if ($gameMessage._align) $gameMessage._align = [];
        }
...
//=============================================================================
// ** Decrypter: Multi-version asset compatibility
//    Accepts either build's encryption key, and remaps assets that the
//    Chinese build ships under a different filename.
//=============================================================================
(function () {
    'use strict';
    ...
    Decrypter._encryptionKeys = [
        "d9b588ffe78772bccacfc02bdf1437e9", // JP / EN
        "e98cfd800a74596dea2c0ccc763d3204" // CN
    ].map(k => k.split(/(.{2})/).filter(Boolean));

    Decrypter._activeKeyIndex = 0;

    Decrypter.readEncryptionkey = function () {
        this._encryptionKey = this._encryptionKeys[this._activeKeyIndex];
    };

    Decrypter.isValidDecryptedHeader = function (bytes) {
        ...
        const isM4a = ascii(4, 4) === 'ftyp';
        return isPng || isOgg || isM4a;
    };

    // Returns the first 16 bytes XORed with whichever key produces a valid
    // file signature, and remembers that key for subsequent assets.
    Decrypter._decryptHeaderBytes = function (original) {
        const keys = this._encryptionKeys;
        const candidate = new Uint8Array(this._headerlength);

        // Last known-good key first, then the rest
        const order = [this._activeKeyIndex];
        for (let i = 0; i < keys.length; i++) {
            if (i !== this._activeKeyIndex)
                order.push(i);
        }
        ...
        for (let k = 0; k < order.length; k++) {
            ...
            for (let i = 0; i < this._headerlength; i++) {
                candidate[i] = original[i] ^ parseInt(key[i], 16);
            }
            if (this.isValidDecryptedHeader(candidate)) {
                this._activeKeyIndex = order[k];
    ...
    Decrypter.decryptArrayBuffer = function (arrayBuffer) {
        ...
        if (arrayBuffer) {
            const original = new Uint8Array(arrayBuffer).slice(0, this._headerlength);
            const candidate = this._decryptHeaderBytes(original);
            for (i = 0; i < this._headerlength; i++)
                view.setUint8(i, candidate[i]);
        }

        return arrayBuffer;
    };

    // Streaming path (AudioStreaming.js)
    // Only the first chunk carries the RPGMV header, the rest pass through.
    Decrypter.decryptUint8Array = function (uint8Array) {
        ...
        uint8Array.set(this._decryptHeaderBytes(uint8Array.slice(0, this._headerlength)), 0);
        return uint8Array;
    };

    //-------------------------------------------------------------------------
    // Filename remap
    //    Key = filename as the JP/EN versions ship it
    //    Value = filename as the Chinese version ships it
    //-------------------------------------------------------------------------

	const RENAMED = new Map([
		['＿k0.rpgmvp', '_k0.rpgmvp'],
		['＿ke.rpgmvp', '_ke.rpgmvp'],
		['＿ke2.rpgmvp', '_ke2.rpgmvp'],
		['＿ki.rpgmvp', '_ki.rpgmvp'],
		['＿ki2.rpgmvp', '_ki2.rpgmvp'],
		['＿wa.rpgmvp', '_wa.rpgmvp'],
		['＿wa2.rpgmvp', '_wa2.rpgmvp'],
		['【hi.rpgmvo', '[hi.rpgmvo'],
		['kokuri_叫び2.rpgmvo', 'kokuri_sakebi2.rpgmvo'],
		['L_★chikagoro.rpgmvo', 'L__chikagoro.rpgmvo'],
		['L_★kimikoso.rpgmvo', 'L__kimikoso.rpgmvo'],
		['L_★ultu.rpgmvo', 'L__ultu.rpgmvo'],
		['MOBU_★oi_nigashitanoka.rpgmvo', 'MOBU__oi_nigashitanoka.rpgmvo'],
		['MOBU_★sokoka.rpgmvo', 'MOBU__sokoka.rpgmvo'],
		['Z_★abunai.rpgmvo', 'Z__abunai.rpgmvo'],
		['Z_★okegagozaimasennka.rpgmvo', 'Z__okegagozaimasennka.rpgmvo'],
		['Z_★watasiha.rpgmvo', 'Z__watasiha.rpgmvo'],
		['【break1】.rpgmvo', '[break1].rpgmvo'],
		['【break2】.rpgmvo', '[break2].rpgmvo'],
		['【break3】.rpgmvo', '[break3].rpgmvo'],
		['【break4】.rpgmvo', '[break4].rpgmvo'],
		['【break5】.rpgmvo', '[break5].rpgmvo'],
		['【break6】.rpgmvo', '[break6].rpgmvo'],
		['【break7】.rpgmvo', '[break7].rpgmvo'],
		['【END】.rpgmvo', '[END].rpgmvo'],
		['【h】se_bukiminoise03.rpgmvo', '[h]se_bukiminoise03.rpgmvo'],
		['【h】se_bukiminoise04.rpgmvo', '[h]se_bukiminoise04.rpgmvo'],
		['【h】se_kiinnoise.rpgmvo', '[h]se_kiinnoise.rpgmvo'],
		['【h】wa.rpgmvo', '[h]wa.rpgmvo'],
		['【izanari】.rpgmvo', '[izanari].rpgmvo'],
		['【izanari】1.rpgmvo', '[izanari]1.rpgmvo'],
		['【izanari】2.rpgmvo', '[izanari]2.rpgmvo'],
		['【tamamo1】.rpgmvo', '[tamamo1].rpgmvo'],
		['【tamamo2】.rpgmvo', '[tamamo2].rpgmvo'],
		['【tamamo3】.rpgmvo', '[tamamo3].rpgmvo'],
		['【tamamo_kohaku】.rpgmvo', '[tamamo_kohaku].rpgmvo'],
		['【tamamo_kohaku2】.rpgmvo', '[tamamo_kohaku2].rpgmvo'],
		['【tamamo_kohaku3】.rpgmvo', '[tamamo_kohaku3].rpgmvo'],
	]);

    Decrypter._resolveUrl = function (url) {
        let decoded;
        try {
        ...
        if (fs.existsSync(path.join(wwwDir, decoded)))
            return url;

        const cut = decoded.lastIndexOf('/') + 1;
        const dir = decoded.slice(0, cut);
        const file = decoded.slice(cut);
        const alt = RENAMED.get(file);

        if (!alt || !fs.existsSync(path.join(wwwDir, dir + alt))) return url;
        return dir + encodeURIComponent(alt);
    };

    const _Decrypter_extToEncryptExt = Decrypter.extToEncryptExt;
    Decrypter.extToEncryptExt = function (url) {
        return Decrypter._resolveUrl(_Decrypter_extToEncryptExt.call(this, url));
    };
...
function okitsuneTranslateCMD(text) {
    ...
    const lang = ConfigManager.getLanguage();
    const cmdTable = $dataTranslations['cmd'];
    const entry = cmdTable && cmdTable[text];
...
function languageValue(values) {
    const value = values[ConfigManager.getLanguage()];
    return value == null ? values['日本語'] : value;
}
...
function tilePatrol(mamono, cornerTiles) {
    if (mamono._target == null) mamono._target = 0;
...
Game_CharacterBase.prototype.onScreen = function() {
...
/*
	...
	* OrangeGreenworks - Switched './greenworks' to './js/libs/greenworks' on line 111 to match Archeia's path.
