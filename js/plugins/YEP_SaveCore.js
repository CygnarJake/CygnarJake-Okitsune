 ...
 *
 * @help
 * Note: I've made modifications for Okitsune.
 * They're at the bottom of the plugin.
 * ============================================================================
 * Introduction
 * ============================================================================
 *
...
Window_SaveInfo.prototype.updateIndex = function() {
  ...
  this._waitTime = 3; // Sets file preview load time in frames. Default was 30.
  this.contents.clear();
};
...
Window_SaveInfo.prototype.drawData = function(data, dx, dy, dw) {
  ...
  } else if (data.match(/STATE[ ]?(\d+)/i)) {
    this.drawState(parseInt(RegExp.$1), dx, dy, dw);
  }
};
...
Window_SaveInfo.prototype.drawLocation = function(dx, dy, dw) {
    ...
    // Translate and trim trailing spaces
    text = Imported["SumRndmDde Translation Engine"] ? SRD.TranslationEngine.translate(text) : text;
    //console.log('[Location Raw Translation]: "' + text + '"');

    // Remove trailing spaces (but keep internal spacing)
    text = text.replace(/\s+$/, '');
    //console.log('[Location Final Trimmed]: "' + text + '"');
...
Scene_File.prototype.onActionLoad = function() {
  // Only show confirmation window if in-game (not from title)
  const isFromTitle = SceneManager._scene instanceof Scene_Load;

  if (!isFromTitle && Yanfly.Param.SaveConfirmLoad) {
...
Scene_File.prototype.performActionSave = function() {
	// Set variable 94 so that we know which file the player saved to
	const savefileId = this.savefileId();
    $gameVariables.setValue(94, savefileId);
...
//=============================================================================
// End of File
// Below are modifications I've made for Okitsune
//=============================================================================

(function() {

	// Overrides the default method to support string values and suppress variable names.
	Window_SaveInfo.prototype.drawVariable = function(id, dx, dy, dw) {
		const rawValue = this._saveContents.variables.value(id);

		// Format the value: apply grouping to numbers, display strings as-is.
		const text = (typeof rawValue === 'number')
		  ? Yanfly.Util.toGroup(rawValue)
		  : String(rawValue);

		// Draw the value centered, without displaying the variable name.
		this.drawText(text, dx, dy, dw, 'center');
	};
  
	Window_SaveInfo.prototype.drawState = function(pos, dx, dy, dw) {
	  // "pos" is 1 for leader, 2 for second party member, etc.
	  var length = this._saveContents.party.maxBattleMembers();
	  var actorId = this._saveContents.party._actors[pos - 1];
	  var member = this._saveContents.actors._data[actorId];
	  if (!member) return;

	  // member._states is an array of state IDs the actor currently has
	  var states = member._states || [];
	  if (states.length === 0) {
		this.drawText('-', dx, dy, dw, 'center');
		return;
	  }

	  // Find the state with the **lowest priority** (lowest priority value = highest priority in RPG Maker)
	  var stateData = states
		.map(function(stateId) { return $dataStates[stateId]; })
		.filter(Boolean)
		.sort(function(a, b) { return a.priority - b.priority; });

	  var best = stateData[0];
...
(function() {
  ...
  // Preserve the original gold count method for fallback.
  const _originalDrawGoldCount = Window_SaveInfo.prototype.drawGoldCount;

  Window_SaveInfo.prototype.drawGoldCount = function(dx, dy, dw) {
    // Retrieve the current gold amount from the save contents.
    const goldValue = Yanfly.Util.toGroup(this._saveContents.party._gold);

    // Get the configured label from plugin parameters (e.g., "%1:", "\i[314] %1:").
    // %1 is replaced with the in-game currency unit.
    const rawLabel = Yanfly.Param.SaveVocabGoldCount;

    // If the label is empty or undefined, fallback to the original method.
    if (!rawLabel) {
      _originalDrawGoldCount.call(this, dx, dy, dw);
      return;
    }

    // Parse escape characters (icon codes, colors, etc.).
    let label = Window_Base.prototype.convertEscapeCharacters(rawLabel);
    label = label.format(TextManager.currencyUnit);

    // Indicate that this section is drawing save data.
    this._drawData = true;

    // Measure label width to offset gold value properly.
    const labelWidth = this.textWidthEx(label);

    // Draw the label on the left (with text codes parsed).
    this.drawTextEx(label, dx, dy);

    this._drawData = false;
    this.changeTextColor(this.normalColor());

    // Draw the gold value aligned to the right, avoiding label overlap.
    this.drawText(goldValue, dx, dy, dw - labelWidth - 4, 'right');
  };
...
(function() {
   ...
   * ---------------------
   * WindowColors: background fill color per save slot (1–7).
   * PictureNames: images to show from /img/pictures/ per slot (1–7).
   * PictureOpacity: how transparent the save background images are (0–255).
   * CropRatio: what % of image width to display (0.0–1.0).
   * ColorCropWidth: how many pixels wide the background color fill should be.
   */
  ...
  var PictureOpacity = 220;
  var CropRatio = 0.755;         // Crop ratio for image width
  var ColorCropWidth = 774;     // Fixed pixel width for background color fill

  /**
   * Refresh override: applies cropped background color and image per slot.
   */
  const _Window_SaveInfo_refresh = Window_SaveInfo.prototype.refresh;
  Window_SaveInfo.prototype.refresh = function() {
    this.contents.clear();

    const slotId = this.savefileId();
    if (slotId >= 1 && slotId <= 7) {
      const colorCode = WindowColors[slotId - 1];
      this.setupCustomBackground(colorCode);

      const picName = PictureNames[slotId - 1];
      this.drawBackgroundPicture(picName);
    } else {
      this.setupCustomBackground(null);
    }

    _Window_SaveInfo_refresh.call(this);
  };

  /**
   * Setup background fill using fixed width cropping.
   */
  Window_SaveInfo.prototype.setupCustomBackground = function(colorCode) {
    ...
    if (colorCode) {
      this._customBackBitmap.fillRect(
        0, 0,
        ColorCropWidth, this.height,
        colorCode
      );
    }
  };
  ...
  /**
   * Draws a cropped background image (cropped to CropRatio % width).
   */
  Window_SaveInfo.prototype.drawBackgroundPicture = function(picName) {
    const bitmap = ImageManager.loadPicture(picName);
    const self = this;

    bitmap.addLoadListener(function() {
      if (!self._customBackBitmap) return;

      const croppedWidth = Math.floor(bitmap.width * CropRatio);
      const croppedHeight = bitmap.height;

      self._customBackBitmap.paintOpacity = PictureOpacity;
      self._customBackBitmap.blt(
        bitmap,
        0, 0,
        croppedWidth, croppedHeight,
        0, 0
      );
      self._customBackBitmap.paintOpacity = 255;
    });
  };
