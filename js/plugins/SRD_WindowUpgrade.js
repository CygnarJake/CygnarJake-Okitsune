...
/* ============================================
 * Okitsu patch — WindowUpgrade: cancel closes
 * (append AFTER SRD_WindowUpgrade.js)
 * - On cancel (ESC/right-click/etc): close the window.
 * - Do NOT write to the choice result variable.
 * - Compatible with Mano_InputConfig (uses 'cancel' symbol).
 * ============================================ */
(function() {
  'use strict';
  if (!window.Window_ChoiceBase || !Window_ChoiceBase.prototype) return;

  // Ensure WindowUpgrade's choice window treats 'cancel' as enabled
  Window_ChoiceBase.prototype.isCancelEnabled = function() { return true; };

  // Provide a close-callback slot if the base plugin didn't define one
  if (!Window_ChoiceBase.prototype.setCloseCallback) {
    Window_ChoiceBase.prototype.setCloseCallback = function(fn) {
      this._closeCallback = fn;
    };
  }

  // Close on cancel without modifying variables (mimics default choice behavior)
  Window_ChoiceBase.prototype.processCancel = function() {
    if (!this.isOpen() || !this.active) return;
    SoundManager.playCancel();
    this.deactivate();
    // Tell the scene we're done (SRD_WindowUpgrade uses this to clear wait mode)
    if (typeof this._closeCallback === 'function') {
      try { this._closeCallback(); } catch (e) {}
    }
    this.close();
    // Intentionally do NOT call any handlers or write to vars.
  };
})();

/* ========================================================================== */
/* Okitsune Patch — QuestionWindow widths like ChoiceWindow                   */
/* Place AFTER SRD_WindowUpgrade.js                                           */
/* ========================================================================== */
(function() {
  'use strict';
  if (!window.Window_ChoiceMessage) return;

  // Use the same defaults as Window_ChoiceBase (cols=1, rows=all).
  var _okitsu_Q_parseData = Window_ChoiceMessage.prototype.parseData;
  Window_ChoiceMessage.prototype.parseData = function(data) {
    data = data || {};
    this._columns   = data.cols || 1;                       // match Choice
    this._rows      = data.rows || this._choices.length;    // match Choice
    this._alignment = data.align || 'center';
  };

  // Size by CHOICES (and columns) exactly like Window_ChoiceBase does.
  Window_ChoiceMessage.prototype.setupWidth = function(width) {
    if (!width) {
      this._width = 0;
      for (var i = 0; i < this._choices.length; i++) {
        var tw = this.textWidth(this._choices[i]);
        if (this._width < tw) this._width = tw * this._columns;
      }
      // Same padding formula used by Choice window
      this._width += (this.standardPadding() * 2)
                   + 48
                   + (this._columns > 1 ? (this.standardFontSize() * this._columns * 2) : 0);
    } else {
      this._width = width;
    }
    this.width = this.windowWidth();
